...
class MNISTStrokesDataSet(DataSet):
    ...
    def next_batch(self, batch_size):
        ...
        Returns:
            points_slice - points as decart coordinats
            inputs_slice - shifts from previous points at every time steps
            targets_slice - one hot encoded targets concatenated with 4x shifts
            labels_slice - usual one hot or not encoding of target
        """
...
class MNISTStrokesDataProvider(DataProvider):
    ...
    def _get_new_data(self, dataset):
        ...
        if self._one_hot:
            dataset_labels = self.labels_to_one_hot(
                dataset_labels, self.n_classes)
        if self._verbose:
